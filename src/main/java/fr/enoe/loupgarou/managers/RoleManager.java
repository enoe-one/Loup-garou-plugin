...
public class RoleManager {
    ...
    private Role createRole(String id, UUID uuid) {
        return switch (id) {
            ...
            case "bienfaiteur"      -> new Bienfaiteur(plugin, uuid);
            case "braconnier"       -> new Braconnier(plugin, uuid);
            case "rebouteux"        -> new Rebouteux(plugin, uuid);
            case "analyste"         -> new Analyste(plugin, uuid);
            // Loups
            case "loup_simple"      -> new LoupSimple(plugin, uuid);
            case "vilain_petit_loup"-> new VilainPetitLoup(plugin, uuid);
            case "loup_hurleur"     -> new LoupHurleur(plugin, uuid);
            case "loup_mesquin"     -> new LoupMesquin(plugin, uuid);
            ...
            // Hybrides
            case "chevaucheur_cochon" -> new fr.enoe.loupgarou.roles.impl.hybride.ChevaucheurDeCochon(plugin, uuid);
            default -> {
    ...
    // ─── MORT D'UN LOUP (GrandMechantLoup + Braconnier) ─────────────────────────

    public void onWolfDeath(UUID deadWolfUUID, org.bukkit.Location deathLoc) {
        // Notifier le Grand Méchant Loup
        roles.values().stream()
            .filter(r -> r instanceof fr.enoe.loupgarou.roles.impl.loup.GrandMechantLoup)
            .map(r -> (fr.enoe.loupgarou.roles.impl.loup.GrandMechantLoup) r)
            .forEach(gml -> gml.onWolfDeath());

        // Notifier le Braconnier
        roles.values().stream()
            .filter(r -> r instanceof fr.enoe.loupgarou.roles.impl.village.Braconnier)
            .map(r -> (fr.enoe.loupgarou.roles.impl.village.Braconnier) r)
            .forEach(b -> b.registerWolfCorpse(deadWolfUUID, deathLoc));
    }
    ...
    public java.util.Collection<fr.enoe.loupgarou.roles.Role> getAllRoles() { return roles.values(); }
    ...
    public boolean hasActedThisNight(UUID u) { return nightActors.contains(u); }

    /**
     * Force le remplacement du rôle d'un joueur (utilisé par SkysofrenieManager).
     * Met à jour wolfTeam si nécessaire.
     */
    public void forceSetRole(UUID uuid, Role newRole) {
        Role old = roles.get(uuid);
        if (old != null && old.getFamily() == RoleFamily.LOUP) wolfTeam.remove(uuid);
        roles.put(uuid, newRole);
        if (newRole.getFamily() == RoleFamily.LOUP) wolfTeam.add(uuid);
    }
